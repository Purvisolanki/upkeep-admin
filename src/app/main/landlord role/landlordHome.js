...
import {
  createProperty,
  getlandordProperties,
  updateProperty,
} from "app/store/landlordHomeSlice";
...
import { deleteProperty } from "../../store/landlordHomeSlice";
...
// const access_token =
//   "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI2NWY0MDc0NDY0NDNhN2IzNmZjMmRiODkiLCJyb2xlIjoiTGFuZGxvcmQiLCJpYXQiOjE3MTA0OTkzNjd9.2q3oZJdCzps2snddZJ-Q_5AKMQYhN9_9WJ9jXwIJnMw";
//access_token
...
function propertyPage(props) {
  ...
  const { landlordProperty, loading } = useSelector((state) => state.landlordHome);
  ...
  useEffect(() => {
    // const access_token =
    //   "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI2NWY0MDc0NDY0NDNhN2IzNmZjMmRiODkiLCJyb2xlIjoiTGFuZGxvcmQiLCJpYXQiOjE3MTA0OTkzNjd9.2q3oZJdCzps2snddZJ-Q_5AKMQYhN9_9WJ9jXwIJnMw";
    dispatch(getlandordProperties(access_token));
  }, []);

  const handleDelete = (propertyId) => {
    // const access_token =
    //   "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI2NWY0MDc0NDY0NDNhN2IzNmZjMmRiODkiLCJyb2xlIjoiTGFuZGxvcmQiLCJpYXQiOjE3MTA0OTkzNjd9.2q3oZJdCzps2snddZJ-Q_5AKMQYhN9_9WJ9jXwIJnMw"; // Replace with your actual access_token
    dispatch(deleteProperty({ access_token, propertyId })).then((res) => {
      res.payload.success && dispatch(getlandordProperties(access_token));
    });
  };
  ...
  const handleUpdate = (editData) => {
    ...
    dispatch(updateProperty({ access_token, editData, updatepropertyId })).then(
      (res) => {
        res.payload.status && dispatch(getlandordProperties(access_token));
      }
    );
    // After successful creation, refresh the property list
    //   dispatch(getlandordProperties(access_token));
      ...
      header={
        ...
        >
          ...
          >
            ...
            {/* {t("Create_property")} */}
          {/* </Button> */} 
          <IconButton  onClick={() => handleClickOpencreate()} style={{  marginRight: "30px" }}
                          color="success"
          ...
          <Container maxWidth="lg" style={{  marginTop: '2%'}}>
            ...
            >
              <Table sx={{ minWidth: 650 }} aria-label="simple table">
                ...
                <TableBody>
                  {landlordProperty && landlordProperty.map((item, index) => (
                    <TableRow key={index}>
